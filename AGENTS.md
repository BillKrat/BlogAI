...
- **Publishing gotcha:** direct SQL upserts do not update BlogEngine's in-memory post cache. Call `POST /api/posts/reload/{blogId}` afterwards. Details: [docs/Copilot-post-cache-reload.md](docs/Copilot-post-cache-reload.md). The publishing SQL scripts live in the `vs-mcp-bridge` repo, which is not in this workspace.
...
- **Branches:** `master-blogai` is the working branch with the real history. `master` is a separate two-commit public snapshot with unrelated history (no common ancestor), and `master_original` is the upstream BlogEngine merge. Which one becomes the trunk is undecided; ask the human before merging across them.
