...
- **SQL schema = Setup.sql + `setup/BillKrat-Upgrade.2026.09.26.sql`** (fork column `be_Users.Comment`, missing from stock Setup.sql; found 2026-09-26 on a clean LocalDB build). Windows on ARM: IIS Express must be x86 (`Use64BitIISExpress` is `false` in the csproj; LocalDB has no ARM64 client).
