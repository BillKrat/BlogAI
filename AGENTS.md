...
**Remaining:** (live schema verified 2026-09-26, see [docs/Claude-live-schema.md](docs/Claude-live-schema.md): not a deployment risk) run the SmarterASP deployment per [docs/Claude-smarterasp-secrets.md](docs/Claude-smarterasp-secrets.md), awaiting the human's go-ahead on the plan (secret values are entered by the human; MCP does verify/restart/health). Rotate the live site's machineKey by deploying without an inline one. The working-tree `Web.Config` currently holds the live deployed copy (real credentials) for comparison: never commit it; stage specific paths only.
