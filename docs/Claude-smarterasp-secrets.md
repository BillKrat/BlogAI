...
| Upload files (config, app) | none | **No MCP file upload exists.** Substitute: `docs/Claude-ftp.ps1` in the workspace root (FTPS, dry run by default, auto-backup of the remote file, refuses a `Web.Config` holding inline secrets). The AI runs it through the vault, so the FTP password never reaches it. Use FTP user `billkrat-adventures` (path `/Adventures`) on `win8166.site4now.net:21`, explicit TLS. |
...
4. **Deploy `Web.Config` safely (AI, via `Claude-ftp.ps1`).** Because the deployed file is the credential-free `Web.Config.SQL` content, there is no server-only `Web.Config` to protect and no round trip of pulling the server copy and undoing it. `put Web.Config.SQL -Remote Web.Config` dry-runs, then with `-Apply` backs up the current server file to `M:\Dev\Websites\Adventures\_ftp-backup\<timestamp>\` and uploads. Upload `connectionStrings.config` the same way (first time only; it is never overwritten by a normal publish because it is not in the project). Application files: Visual Studio Web Deploy or the same tool. Never commit publish profiles with passwords.
