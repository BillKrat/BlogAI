...
| Web Deploy state (**not used for BlogAI**) | `deployment_get_webdeploy_settings`, `deployment_disable_webdeploy` | The human removed all publish profiles on 2026-09-26; deployment is FTP only. **Web Deploy is DISABLED on the host** (`deployment_disable_webdeploy`, operation `7dec679b-b9b1-48c7-9377-1367eb510633`, 2026-09-26); to re-enable: `deployment_enable_webdeploy` plus a new credential. |
...
## The 2026-08-15 "deploy Error" entries (resolved as not-a-problem, 2026-09-26)
Two failed runs of SmarterASP's GitHub-driven build automation (a Docker/railpack pipeline). Each cloned the repository and stopped within seconds: `Could not identify a project root by (*.csproj, package.json). Project is not based on dotnet or node.` That pipeline builds .NET (Core) or Node projects whose project file is at the repo root; BlogAI is a classic .NET Framework solution with the web project under `BlogEngine/BlogEngine.NET`, so it can never match. It failed at detection, before touching any site file, so there was no impact. **Do not use the GitHub deploy function for BlogAI**; deploy with `Claude-ftp.ps1` (or Web Deploy). Leftovers on the server: `Adventures/node_app_automate_deploy_3701963.log` and `..._3702017.log` (1,380 bytes each; not served publicly, HTTP 404; they only name an internal path). Deleted 2026-09-26 with `Claude-ftp.ps1 delete` (local copies in `M:\Dev\Websites\Adventures\_ftp-backup\`).
