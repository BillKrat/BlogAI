...
| Web Deploy state (**not used for BlogAI**) | `deployment_get_webdeploy_settings`, `deployment_disable_webdeploy` | The human removed all publish profiles on 2026-09-26; deployment is FTP only. Web Deploy is still enabled on the host; consider disabling it to shrink the attack surface (needs the human's go-ahead). |
